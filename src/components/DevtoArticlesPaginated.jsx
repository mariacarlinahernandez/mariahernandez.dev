  ...
  render() {
    ...
    if (!totalRecords) return (
      <div className="post-feed inner-wide">
        <p className="info">No post from Dev.to obtained.</p>
      </div>
    )
    ...
    const articlesToRender = this.state.devtoArticles.slice(offset, offset + pageLimit)
      ...
      <div className="post-feed inner-wide">
        ...
        <div className={style.pagination}>
          {pages.map((page, index) => {
            ...
            return (
              <span key={index} className={classes} onClick={() => this.changePage(page)}>{page}</span>
            )
          })}
        </div>
      </div>
